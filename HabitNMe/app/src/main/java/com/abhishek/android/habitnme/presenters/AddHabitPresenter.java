...
//            }
    ...
    public void updateHabit(final long id, final String name, final long date, final String category , final String description, final int max, final int min,
                            final int type, final int timesAWeek) {
        final Observable<Boolean> observable = Observable.create(new Observable.OnSubscribe<Boolean>() {
            @Override
            public void call(Subscriber<? super Boolean> subscriber) {
                try {
                    update(id, name, date, category, description, max, min, type, timesAWeek);
                    subscriber.onNext(true);
                    subscriber.onCompleted();
                } catch (Exception e) {
                    subscriber.onError(e);
        ...
        restartableLatestCache(EDIT_DATA, new Func0<Observable<Boolean>>() {
            @Override
            public Observable<Boolean> call() {
                return observable;
            }
    ...
    public void delete(final long id) {
        ...
        restartableLatestCache(DEL_DATA, new Func0<Observable<Boolean>>() {
            @Override
            public Observable<Boolean> call() {
                return observable;
            }
    ...
    public void deleteHabit(long id) throws  Exception {
        Realm realm = Realm.getInstance(realmConfiguration);
        try {
            realm.beginTransaction();
            RealmResults<HabitModel> realmResults =  realm.where(HabitModel.class).equalTo("id", id).findAll();
            realmResults.first().deleteFromRealm();
            RealmResults<HabitDayLog> realmResultsLog =  realm.where(HabitDayLog.class).equalTo("habitModel.id", id).findAll();
            for (HabitDayLog dayLog:
                 realmResultsLog) {
                dayLog.deleteFromRealm();
            }
            realm.commitTransaction();
        } catch (Exception e) {
    ...
    private void update(long id,String name, long date, String category , String description, int max, int min,
                        int type, int timesAWeek) throws  Exception{
        Realm realm = Realm.getInstance(realmConfiguration);
        try {
            realm.beginTransaction();
